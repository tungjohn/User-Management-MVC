...
<div class="d-flex justify-content-center align-items-center" style="min-height: 80vh;">
    <form class="row g-3" style="max-width: 700px; width: 100%;" method="post" id="form-user" action="/users/store">
        <div class="col-md-12">
            <label for="name" class="form-label">Tài khoản</label>
            <input type="text" name="name" class="form-control mx-auto" id="name" value="{{ old('name') }}" required>
            {! form_error('name', '<span style="color: red;">', '</span>') !}
        </div>
        <div class="col-md-12">
            <label for="email" class="form-label">Email</label>
            <input type="email" name="email" class="form-control mx-auto" id="email" placeholder="name@example.com" value="{{ old('email') }}" required>
        ...
        <div class="col-md-12">
            ...
            <select class="form-select mx-auto" name="group_id" id="validationDefault04" required>
                ...
                @foreach ($groups as $group)
                    <option value="{{$group['id']}}" {{!empty(old('group_id')) && old('group_id') == $group['id'] ? 'selected' : ''}}>{{$group['name']}}</option>
                @endforeach
            </select>
        ...
        <div class="col-md-12">
            ...
            <select class="form-select mx-auto" id="status" name="status">
                @foreach ($status as $key => $value)
                    <option value="{{$key}}" {{!empty(old('status')) && old('status') == $key ? 'selected' : ''}}>{{$value}}</option>
                @endforeach
            </select>
